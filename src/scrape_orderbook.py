...
async def save_state(buffer: List[Dict], file_path: str):
    ...
    if current_time - last_log_time >= LOG_INTERVAL:
        logger.info(f"Buffered data saved to {file_path}.")
...
async def regular_snapshot(manager, aggtrade_manager, buffered_data_manager, file_path):
    while True:
        ...
        data = {
            ...
            "bids": ujson.dumps(snapshot_state["bids"]),
            "asks": ujson.dumps(snapshot_state["asks"]),
            "is_snapshot": snapshot_state["is_snapshot"],
            "last_update_id": snapshot_state["last_update_id"],
        ...
        buffer = buffered_data_manager.add_data(data)
        if buffer:
            await save_state(buffer, file_path)
        
        await asyncio.sleep(SNAPSHOT_INTERVAL)

async def check_for_new_day(symbol, manager, aggtrade_manager, buffered_data_manager, file_path):
    ...
    while True:
        ...
        if now.date() > current_date:
            ...
            initial_data = {
                "timestamp": pd.Timestamp.now(tz='UTC').floor('ms'),
                "bids": ujson.dumps(initial_snapshot["bids"]),
                "asks": ujson.dumps(initial_snapshot["asks"]),
                "is_snapshot": initial_snapshot["is_snapshot"],
                "last_update_id": initial_snapshot["last_update_id"],
                "buy_volume": aggtrade_data["buy_volume"],
                "sell_volume": aggtrade_data["sell_volume"],
                "vwap": aggtrade_data["vwap"],
                "total_trades": aggtrade_data["total_trades"]
            ...
            buffered_data_manager.add_data(initial_data)
            await save_state([initial_data], new_file_path)
            
            logger.info(f"Started new file for {symbol}: {new_file_name}")
            return new_file_path  # Return the new file path


async def handle_websocket(websocket, symbol, manager, aggtrade_manager, buffered_data_manager, file_path):
    ...
    try:
        while True:
            try:
                ...
                # Check if it's time to save the buffer (every minute)
                if current_time - last_save_time >= 60:
                    buffer = buffered_data_manager.get_buffer()
                    if buffer:
                        await save_state(buffer, file_path)
                        last_save_time = current_time
...
async def maintain_websocket_connection(symbol, manager, aggtrade_manager, buffered_data_manager, file_path):
    ...
    while True:
        try:
            async with websockets.connect(url) as websocket:
                ...
                reconnect_attempt = 0
                reconnect_delay = INITIAL_RECONNECT_DELAY
                await handle_websocket(websocket, symbol, manager, aggtrade_manager, buffered_data_manager, file_path)
        except Exception as e:
...
async def main(symbol: str):
    ...
    buffered_data_manager = BufferedDataManager()
    current_date = datetime.now(timezone.utc).date()
    ...
    snapshot_task = asyncio.create_task(regular_snapshot(manager, aggtrade_manager, buffered_data_manager, file_path))
    new_day_task = asyncio.create_task(check_for_new_day(symbol, manager, aggtrade_manager, buffered_data_manager, file_path))
    ...
    try:
        ...
        initial_data = {
            ...
            "is_snapshot": initial_snapshot["is_snapshot"],
            "last_update_id": initial_snapshot["last_update_id"],
            "buy_volume": initial_aggtrade_data["buy_volume"],
            "sell_volume": initial_aggtrade_data["sell_volume"],
            "vwap": initial_aggtrade_data["vwap"],
            "total_trades": initial_aggtrade_data["total_trades"]
        }
        buffered_data_manager.add_data(initial_data)
        await save_state([initial_data], file_path)

        websocket_task = asyncio.create_task(maintain_websocket_connection(symbol, manager, aggtrade_manager, buffered_data_manager, file_path))
